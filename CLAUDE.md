...
- **Activity log** (`lib/activity-log.ts`, page at `/logs`): internal-only, not shown to merchants — one entry per non-demo report fetch (account, report type, range, success/error, duration), so the team can see whether a given merchant is actually opening the app. Backed by Vercel KV (the app's only persistent storage — everything else is fetched live from Meta or kept client-side); logging is fire-and-forget and never throws into the report response if KV isn't configured or a write fails. `/logs` is listed in `middleware.ts`'s `PUBLIC_PATHS` — intentionally reachable with no login, so it can be shared as a plain link without handing out the merchant-facing password. JSON reports log via `next/server`'s `after()` (runs post-response, doesn't delay the client); streaming reports log via an `onSettled` callback passed into `ndjsonResponse()` (`lib/stream.ts`), since their response starts streaming before the work finishes.
