  ...
  return (
    <div className="w-64">
      <button
        ...
        className="mb-2 w-full rounded-md py-1 text-center text-xs font-semibold text-ink-secondary transition-colors hover:bg-surface-app"
      >
        {formatMonthTitle(monthStart)}
      </button>
      <div className="grid grid-cols-7 gap-y-0.5">
        {WEEKDAY_LABELS.map((w) => (
          <div key={w} className="flex h-7 items-center justify-center text-[10px] font-semibold uppercase tracking-wide text-ink-tertiary">
            {w}
          </div>
        ))}
        {cells.map((cell) => {
            ...
            <div key={cell.date} className="relative h-8">
              ...
              <button
                ...
                className={`relative z-10 flex h-8 w-8 items-center justify-center rounded-full text-xs tabular-nums transition-colors ${
                  cell.disabled
                    ? "cursor-not-allowed text-ink-tertiary opacity-30"
                    : !cell.inMonth
                    ...
                    : "text-slate-700 hover:bg-surface-app"
                }`}
              >
                {cell.day}
              </button>
...
export function DateRangePicker({ value, onChange }: DateRangePickerProps) {
  ...
  return (
    <div ref={rootRef} className="relative flex flex-col items-end gap-1.5">
      <div className="flex items-center gap-1.5">
        ...
        <button
          ...
          className={`flex h-9 items-center gap-2 rounded-lg px-3.5 text-[13px] font-semibold transition-colors ${
            menuOpen
              ? "text-ink ring-2 ring-brand-500/40"
              : value
              ? "text-ink hover:opacity-85"
              : "text-white hover:opacity-85"
          }`}
          style={value || menuOpen
            ? { background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)" }
            : { background: "linear-gradient(135deg, #AF46FD, #DA3BC2, #F4349D)", border: "1px solid transparent" }
          }
        >
          <CalendarIcon className={value || menuOpen ? "text-brand-500" : "text-white/80"} />
          {value ? (
            <span className="flex items-baseline gap-1.5">
              {activeMonths !== null && (
                <span className="text-ink-tertiary">Previous {activeMonths} Month{activeMonths > 1 ? "s" : ""} ·</span>
              )}
          ...
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={`transition-transform ${menuOpen ? "rotate-180" : ""} ${value || menuOpen ? "text-ink-tertiary" : "text-white/70"}`}>
            <path d="m6 9 6 6 6-6" />
          </svg>
        </button>
      </div>
      ...
      {menuOpen && (
        <div className="absolute right-0 top-full z-50 mt-1.5 overflow-hidden rounded-xl"
        ...
        >
          {!customOpen ? (
            <div className="max-h-[420px] w-[19rem] overflow-y-auto py-1.5">
              <button
                onClick={openCustom}
                className="flex w-full items-center gap-2 px-3.5 py-2.5 text-left text-[13px] font-medium text-ink-secondary transition-colors hover:bg-surface-app"
              >
                ...
                return (
                  ...
                  >
                    ...
                    <span className="flex items-center gap-2">
                      <span className="text-[11px] tabular-nums text-ink-tertiary">{formatFullRange(r)}</span>
                      {selected && (
            ...
            <div className="w-[34rem] p-3">
              <div className="mb-2 flex items-center justify-between">
                <button
                  onClick={backToList}
                  className="flex items-center gap-1 rounded-md px-1.5 py-1 text-xs font-medium text-ink-tertiary transition-colors hover:bg-surface-app hover:text-ink-secondary"
                >
                ...
                {pickerLevel === "days" && (
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="rounded-md p-1 text-ink-tertiary transition-colors hover:bg-surface-app hover:text-ink-secondary disabled:opacity-30 disabled:pointer-events-none"
                    >
                      <ChevronLeft className="h-3.5 w-3.5" />
                    </button>
                    <span className="text-xs font-medium text-ink-tertiary">{viewMonth && `${formatMonthTitle(viewMonth)} – ${formatMonthTitle(rightMonth)}`}</span>
                    <button
                      onClick={goNextMonth}
